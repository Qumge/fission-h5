<template>
	<view class="page" style="border: 1px solid red;background: #fff;">
		<!--header-->
		<view class="tui-header-box" :style="{ height: headerHeight + 'px;' }" style="color:#fff">
			<view class="tui-header" :style="{ paddingTop: top + 'px' }">老虎机</view>
		...
		<!-- <image style="position: absolute;top: 0px;width: 100%;height: 80%;" src="/static/images/game/stage.png"></image> -->
		<view>
			<view class="slot-machine">
				...
				/>
				<view class="mp-slot">
					<view class="mp-slot-box">
						<view v-for="(item, some) in num" class="slot-group" :key="some" :style="{ transform: 'translateY(' + transY1 + 'px)' }">
							<view class="slot-item">{{ item }}</view>
						</view>
					</view>

					<view class="mp-slot-box">
						<view v-for="(item, some) in num" class="slot-group" :key="some" :style="{ transform: 'translateY(' + transY2 + 'px)' }">
							<view class="slot-item">{{ item }}</view>
						</view>
					</view>

					<view class="mp-slot-box">
						<view v-for="(item, some) in num" class="slot-group" :key="some" :style="{ transform: 'translateY(' + transY3 + 'px)' }">
							<view class="slot-item">{{ item }}</view>
						</view>
					</view>
				</view>
...
export default {
	...
	data() {
		return {
			...
			opcity: 0,
			iconOpcity: 0.5,
			prize_numbers: ['【777】', '【888】', '【666】', '【其他三连数字】'],
			prizeNumbers: [[7,7,7], [8,8,8], [6, 6,6], [3, 3, 3]]
		};
	},
	onLoad(options) {
		// 12
	...
	methods: {
		...
		start() {
			...
			if (this.userIntegral < this.game.cost) {
				uni.showModal({
					title: '温馨提示',
					content: '您的金币不足'
				});
					...
					.then(function(data) {
						...
						if (data.error) {
							...
							if(data.prize_log){
								...
								arr = that.getRandomArrayElements([1,2,3,4,5,6,7,8,9], 3)
							}
							that.num1 = arr[0]
							that.num2 = arr[1]
							that.num3 = arr[2]
							console.log(arr);
							that.changeColor = 0.1;
							let { isStart, len, height, transY1, transY2, transY3, transY4, speed, num1, num2, num3, num4, endCallBack } = that;
							if (isStart) return;
							that.isStart = true;
							const totalHeight = height * len;
							const sRange = Math.floor(Math.random() * 2 + 2);
							const halfSpeed = speed / 2;
							const endDis1 = that.num1 == 0 ? 10 * height : num1 * height;
